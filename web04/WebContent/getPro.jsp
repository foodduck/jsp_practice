...
<%
	ProlistVO pro = (ProlistVO) request.getAttribute("pro");
%>
<!DOCTYPE html>
<html>
<head>
		...
		<form action="UpdateProCtrl" method="post" name="frm">
			<table class="table">
				<tr>	
					...
					<td class="item2">
						<input type="hidden" name="proid" id="proid" value="<%=pro.getProid() %>" />
						<input type="text" name="protype" id="protype" value="<%=pro.getProtype() %>" />
					</td>
				</tr>
				<tr>	
					<th class="item1">상품명</th>
					<td class="item2">
						<input type="text" name="proname" id="proname" value="<%=pro.getProname() %>" />
					</td>
				</tr>
				<tr>
					<th class="item1">상품설명</th>
					<td class="item2">
						<textarea name="procom" id="procom"  cols="90" rows="10"><%=pro.getProcom() %></textarea>
					</td>
				</tr>
				<tr>	
					<th class="item1">가격</th>
					<td class="item2">
						<input type="text" name="price" id="price" value="<%=pro.getPrice() %>" />
					</td>
				</tr>
				<tr>	
					<th class="item1">수량</th>
					<td class="item2">
						<input type="number" name="cnt" id="cnt" max="100" min="1" value="<%=pro.getCnt() %>" />
					</td>
				</tr>
				<tr>	
					<th class="item1">상품 이미지</th>
					<td class="item2">
						<input type="text" name="imgdata" id="imgdata"  value="<%=pro.getImgData() %>" />
						<input type="button" class="btn dtn-default" onclick="imgCheck()" value="이미지 업로드"/><br>
						<img src="<%=pro.getImgData() %>" id="proData" alt="제품사진" />
