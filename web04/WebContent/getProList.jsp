...
<%
	ArrayList<ProlistVO> proList = (ArrayList<ProlistVO>) request.getAttribute("proList");
%>    
<!DOCTYPE html>
<html>
<head>
	...
	<div class="content">
		...
		%>
				<tr>
					...
					<td class="item2"><%=pro.getProtype() %></td>
					<td class="item3"><%=pro.getProname() %></td>
					<td class="item4"><%=pro.getProcom() %></td>
					<td class="item5"><%=pro.getPrice() %></td>
					<td class="item6"><%=pro.getCnt() %></td>
					<td class="item7">
						<a href="GetProCtrl?proid=<%=pro.getProid() %>" class="btn btn-default">편집</a>
						<a href="DeleteProCtrl?proid=<%=pro.getProid() %>" class="btn btn-default">삭제</a>
					</td>
				</tr>
		<%
		}
