...
public class InformDAO2 {
	...
	public List<InformVO> getList(int startRow, int endRow) {
		...
		try {
			...
			String sql = "select * from (select rownum as rn, seq, nickname, title, incontent, regdate, grade, userid, visited from (select * from inform order by seq desc)) where rn between ? and ?";
			...
			if (rs.next()) { // 데이터베이스에 데이터가 있으면 실행
				...
				do {
					...
					list.add(inform); // list에 0번 인덱스부터 inform 객체의 참조값을 저장
				} while (rs.next());
			}
		} catch (Exception e) {
			e.printStackTrace();
