...
public class ProlistDAO {
		...
		public ArrayList<ProlistVO> getProList() { 
			...
			try {
				...
				String sql = "select * from prolist2 order by proid desc, protype asc";
...
//				stmt.close();
//				if(num>0) {
					sql = "select * from prolist2 where proid = ?";
					stmt = con.prepareStatement(sql);
					stmt.setInt(1, vo.getProid());
					rs = stmt.executeQuery();
					if(rs.next()) {
...
//					}
		...
		public int updatePro(ProlistVO vo) {
			...
			try {
				con = DBConn.getConnection();
				String sql = "update prolist2 set protype=?, proname=?, procom=?, price=?, cnt=?, imgdata=? where proid=?";
				stmt = con.prepareStatement(sql);
		...
		public int deletePro(ProlistVO vo) {
			...
			try {
				con = DBConn.getConnection();
				String sql = "delete from prolist2 where proid=?";
				stmt = con.prepareStatement(sql);
				stmt.setInt(1, vo.getProid());
				num = stmt.executeUpdate();
			} catch(Exception e) {
		...
		public int addPro(ProlistVO vo) {
			...
			try {
				con = DBConn.getConnection();
				String sql = "insert into prolist2 values(prolist_seq.nextval, ?, ?, ?, ?, ?, ?, 0)";
				stmt = con.prepareStatement(sql);
				stmt.setString(1, vo.getProtype());
				stmt.setString(2, vo.getProname());
				stmt.setString(3, vo.getProcom());
