...
public class ProlistDAO2 {
		...
		public List<ProlistVO> getProList(int startRow, int endRow) { 
			...
			try {
				...
				String sql = "select * from (select rownum as rn, proid, protype, proname, procom, price, cnt, imgdata, visited from (select * from prolist2 order by proid desc)) where rn between ? and ?";
		...
		public int getCount(){
			int count = 0;
			String sql = "select count(*) from prolist2";
			try {
				con = DBConn.getConnection();
				stmt = con.prepareStatement(sql);
				rs = stmt.executeQuery();
		...
		public ProlistVO getPro(ProlistVO vo) {
			...
			try {
				con = DBConn.getConnection();
				String sql = "update prolist2 set visited=visited+1 where proid = ?";
				stmt = con.prepareStatement(sql);
				stmt.setInt(1, vo.getProid());
				int num = stmt.executeUpdate();
				stmt.close();
				if(num>0) {
					sql = "select * from prolist2 where seq = ?";
					stmt = con.prepareStatement(sql);
					stmt.setInt(1, vo.getProid());
					rs = stmt.executeQuery();
					if(rs.next()) {
